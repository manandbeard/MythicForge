...
export const generateEncounter = async (partyLevel: number, environment: string, theme: string = "High Fantasy D&D 5e") => {
  const prompt = `Generate a D&D 5e random encounter for a party with an average level of ${partyLevel} in a ${environment} environment. The setting theme is ${theme}.

  Return JSON with:
  - title: A cool title for the encounter.
  - description: A short flavor text describing what the players see.
  - monsters: An array of objects: { name: string, count: number, short_desc: string }
  - nonCombat: A non-combat encounter, hazard, or discovery (string).
  - environmentalChallenge: A specific challenge related to the terrain (string).`;
          ...
          properties: {
            ...
            monsters: {
              ...
              items: {
                ...
                required: ["name", "count", "short_desc"]
              }
            },
            nonCombat: { type: Type.STRING },
            environmentalChallenge: { type: Type.STRING }
          },
          required: ["title", "description", "monsters", "nonCombat", "environmentalChallenge"]
        }
      }
    });
